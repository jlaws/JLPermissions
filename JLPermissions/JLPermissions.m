...
{
    [self authorizeMicrophoneWithTitle:[self defaultTitle:@"Microphone"] message:[self defaultMessage] cancelTitle:[self defaultCancelTitle] grantTitle:[self defaultGrantTitle] completionHandler:completionHandler];
}

- (void) authorizeMicrophoneWithTitle:(NSString *)messageTitle message:(NSString *)message cancelTitle:(NSString *)cancelTitle grantTitle:(NSString *)grantTitle completionHandler:(AuthorizationBlock)completionHandler
{
...
{
    [self authorizeHealthWithTitle:[self defaultTitle:@"Health"] message:[self defaultMessage] cancelTitle:[self defaultCancelTitle] grantTitle:[self defaultGrantTitle] completionHandler:completionHandler];
}

- (void) authorizeHealthWithTitle:(NSString *)messageTitle message:(NSString *)message cancelTitle:(NSString *)cancelTitle grantTitle:(NSString *)grantTitle completionHandler:(AuthorizationBlock)completionHandler
{
...
{
    NSString *title = [NSString
                       stringWithFormat:@"\"%@\" Would Like Access to Facebook Accounts",[self appName]];
    [self authorizeFacebookWithTitle:title message:[self defaultMessage] cancelTitle:[self defaultCancelTitle] grantTitle:[self defaultGrantTitle] completionHandler:completionHandler];
}

- (void) authorizeFacebookWithTitle:(NSString *)messageTitle message:(NSString *)message cancelTitle:(NSString *)cancelTitle grantTitle:(NSString *)grantTitle completionHandler:(AuthorizationBlock)completionHandler
{
    BOOL authorized = [self facebookAuthorized];
    bool previouslyAsked = [[NSUserDefaults standardUserDefaults] boolForKey:kJLAskedForFacebookPermission];
    if(authorized)
    {
        completionHandler(true, nil);
    }
...
{
    HKHealthStore *healthStore = [[HKHealthStore alloc] init];
    [healthStore requestAuthorizationToShareTypes:self.healthWriteTypes readTypes:self.healthReadTypes completion:^(BOOL success, NSError *error)
     {
         if(success)
         {
    ...
    [accountStore requestAccessToAccountsWithType:accountType options:options completion:^(BOOL granted, NSError *error)
     {
         dispatch_async(dispatch_get_main_queue(), ^{
            if(granted)
            {
             ...
             {
                 self.facebookCompletionHandler(false, [self systemDeniedError:error]);
             }
             else
             {
                 self.facebookCompletionHandler(false, [self previouslyDeniedError]);
             }
         });
     }];
}
